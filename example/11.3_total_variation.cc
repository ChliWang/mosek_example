...
Model::t total_var(int n, int m) {
  ...
  Variable::t ucore = u->slice(new_array_ptr<int, 1>({0, 0}), new_array_ptr<int, 1>({n, m})); // 0~n-1 0~m-1
  ...
  M->constraint( Expr::stack(2, t, deltax, deltay), Domain::inQCone()->axis(2) );//t deltax deltay都是 n * m
