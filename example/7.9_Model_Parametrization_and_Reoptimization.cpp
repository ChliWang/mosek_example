...
int smallExample() 
{
  //! 关于norm_1 norm_2 和 Qcone 的优化问题的转换
  ...
  auto M = initializeModel(m, n, A); //* 确定模型结构
  ...
  std::cout << "dualObjValue " << M->dualObjValue() << std::endl;
  ...
  sol = x->level();
  std::cout << "Objective " << M->primalObjValue() << ", solution " << (*sol)[0] << ", " << (*sol)[1] << "\n"; 
  std::cout << "dualObjValue " << M->dualObjValue() << std::endl; // 拉格朗日对偶问题的值, 符合约束条件下是等同原问题
  // And increase lamda2
