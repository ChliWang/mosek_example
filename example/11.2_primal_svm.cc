...
int main(int argc, char ** argv)
{
  ...
  M->constraint( Expr::add(Expr::mulElm( y, ex ), xi ) , Domain::greaterThan( 1. ) ); //? 逐元素相乘

  M->constraint( Expr::vstack(1., t, w) , Domain::inRotatedQCone() ); //? 如果是sqruare_norm 用inRotatedQCone更方便

  std::cout << "   c   | b      | w\n"; //? c是 权重，w b 表明了分离直线的位置
  for (int i = 0; i < nc; i++)
  {
    double c = 500.0 * i;
